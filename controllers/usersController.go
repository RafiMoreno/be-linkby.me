...
func Login(c *gin.Context) {
	...
	isHttp, err := strconv.ParseBool(os.Getenv("COOKIE_HTTP_ONLY"))

	if err != nil {
		isHttp = true
	}

	if os.Getenv("COOKIE_SAME_SITE") == "strict" {
		c.SetSameSite(http.SameSiteStrictMode)
	} else if os.Getenv("COOKIE_SAME_SITE") == "lax" {
		c.SetSameSite(http.SameSiteLaxMode)
	} else if os.Getenv("COOKIE_SAME_SITE") == "none" {
		c.SetSameSite(http.SameSiteNoneMode)
	} else {
		c.SetSameSite(http.SameSiteDefaultMode)
	}

	c.SetCookie("token", tokenString, 3600*24*30, "/", os.Getenv("FE_DOMAIN"), isSecure, isHttp)
